...
<input formaction="${pageContext.request.contextPath}/XuqiuServlet?method=select" name="submit" value="查询" type="submit" style="width:60px;height:25px"><br><br>
<script>
function tianjia(){
	for(id=1;id<5;id++){
...
<c:forEach var="item" items="${xuqiuBean1}" varStatus="status">
    		...
    		<a href="#" style="color:blue;text-decoration:none">修改</a>
    		</c:if>
    		<c:if test="${item.status=='已提交'||item.status=='1'}">
    		<a href="${pageContext.request.contextPath}/XuqiuServlet?method=delete&xuqiuname=${item.mingcheng}" style="color:blue;text-decoration:none">删除</a></td>
    		</c:if>
    		<c:if test="${item.status=='已通过审核'}">
    		<a href="${pageContext.request.contextPath}/XuqiuServlet?method=ck&xuqiuname=${item.mingcheng}" style="color:blue;text-decoration:none">查看</a>
    		</c:if>
